  ...
  try {
    ...
    const consolidation = await queryOne<any>(
      `SELECT 
        ...
        (SELECT COUNT(*) 
         FROM Tickets t 
         WHERE t.flight_schedule_id = fc.original_flight_schedule_id
           AND t.status NOT IN ('Cancelled','Moved')
        ) as remaining_active_passengers_on_original
    ...
    if (!consolidation) return notFoundResponse('Flight consolidation not found');
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!existing) return notFoundResponse('Flight consolidation not found');
    ...
    await query(`UPDATE Flight_consolidation SET ${updates.join(', ')} WHERE consolidation_id = ?`, values);
    ...
    return successResponse(updated, 'Flight consolidation updated successfully');
  } catch (error: any) {
